    ...
    if test is None:
        ...
        if which_type == CODE_TYPE.call_based:
            ...
            try:
                ...
                # error_info = {"name": type(e).__name__, "value": str(e)}
        ...
        for index, inputs in enumerate(in_outs["inputs"]):
            ...
            if which_type == CODE_TYPE.call_based:  # Call-based
                ...
                try:
                    ...
                    tb_info = re.sub(r'\(.*?\)', '', tb_info).strip()
                    tb_info = re.sub(r'.* return _method\n', '', tb_info, flags=re.DOTALL)
                    print("str(tb_info): ",str(tb_info))
                    if(type(e).__name__ == 'UnboundLocalError'):
                        error_info = {"name": type(e).__name__, "value": str(e)}
                    else:
                        error_info = {"name": type(e).__name__, "value": tb_info}
                    errors.append(error_info) 
                    results.append(-1)
